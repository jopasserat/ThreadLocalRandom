...
public class ArithmeticMod  {
   ...
   public static void multMatVect(double[] v, double[][] A, double m1,
                                   double[][] B, double m2) {
      double[] vv = new double[3];
        System.arraycopy(v, 0, vv, 0, 3);
      ArithmeticMod.matVecModM(A, vv, vv, m1);
        System.arraycopy(vv, 0, v, 0, 3);

      for(int i = 0; i < 3; i++)
         vv[i] = v[i + 3];
      ArithmeticMod.matVecModM(B, vv, vv, m2);
        System.arraycopy(vv, 0, v, 3, 3);
   }
